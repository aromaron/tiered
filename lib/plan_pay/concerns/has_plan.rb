...
module PlanPay
  module Concerns
    module HasPlan
      ...
      included do
        has_many :plan_pay_assignments,
                 class_name: 'PlanPay::Models::Assignment',
                 as: :plan_owner,
                 dependent: :destroy

        has_many :plan_pay_quota_states,
                 class_name: 'PlanPay::Models::QuotaState',
                 as: :plan_owner,
                 dependent: :destroy

        has_many :plan_pay_usages,
                 class_name: 'PlanPay::Models::Usage',
                 as: :plan_owner,
                 dependent: :destroy
      end
      ...
      def quota_severity(quota_key)
        ...
        quota_state = plan_pay_quota_states.find_by(quota_key: quota_key)

        if quota_state&.blocked?
          :blocked
        elsif quota_state&.exceeded_at
          :grace
        else
          :warning
        end
